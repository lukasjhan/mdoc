...
export const vJarmResponseMode = v.picklist([
  'jwt',
  'query.jwt',
  'fragment.jwt',
  'form_post.jwt',
...
export type Openid4vpJarmResponseMode = v.InferInput<
  typeof vOpenid4vpJarmResponseMode
>;
...
export const getDefaultResponseMode = (input: {
  response_type: ResponseTypeOut;
}): 'query' | 'fragment' => {
  const { response_type } = input;
...
export const getJarmDefaultResponseMode = (input: {
  response_type: ResponseTypeOut;
...
export const validateResponseMode = (input: {
  response_type: ResponseTypeOut;
  ...
  if (disallowedResponseModes?.includes(input.response_mode)) {
    throw new Error(
      `Response_type '${input.response_type}' is not compatible with response_mode '${input.response_mode}'.`
    );
  }
};
