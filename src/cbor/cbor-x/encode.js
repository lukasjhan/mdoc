import {
  addExtension as decodeAddExtension,
  Decoder,
  FLOAT32_OPTIONS,
  mult10,
  Tag,
...
export class Encoder extends Decoder {
    ...
    let encodeUtf8 = ByteArray.prototype.utf8Write
      ...
      : textEncoder && textEncoder.encodeInto
        ? function (string, position) {
            return textEncoder.encodeInto(string, target.subarray(position))
              .written;
          }
        : false;
    ...
    this.encode = function (value, encodeOptions) {
      ...
      if (options.pack) {
        ...
        packedValues.maxValues =
          ...
          (sharedPackedObjectMap ? 16 : Infinity);
      ...
      try {
        ...
        if (referenceMap && referenceMap.idsToInsert) {
          ...
          let serialized = insertIds(
            target.subarray(start, position),
            referenceMap.idsToInsert
          );
          referenceMap = null;
          return serialized;
        }
      ...
      if (type === 'string') {
        ...
        if (bundledStrings && strLength >= 4 && strLength < 0x400) {
          if ((bundledStrings.size += strLength) > MAX_BUNDLE_SIZE) {
            ...
            let maxBytes =
              (bundledStrings[0]
                ? bundledStrings[0].length * 3 + bundledStrings[1].length
                : 0) + 10;
          ...
          if (
            ...
            if (
              ...
              (xShifted =
                value *
                mult10[
                  ((target[position] & 0x7f) << 1) | (target[position + 1] >> 7)
                ]) >>
                0 ===
                xShifted
            ) {
        ...
        : (object, skipValues) => {
            ...
            if (recordId !== undefined) {
              ...
              if (recordId < maxSharedStructures) {
                ...
                for (let i = 0; i < length; i++) {
                  if (
                    transition[RECORD_SYMBOL] === undefined ||
                    transition[RECORD_SYMBOL] & 0x100000
                  )
    ...
    function* encodeObjectAsIterable(object, iterateProperties, finalIterable) {
      ...
      if (constructor === Object) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (
            value &&
            (typeof value === 'object' || position - start > chunkThreshold)
          ) {
  ...
  updateSharedData() {
    ...
    if (saveResults === false) {
      ...
      // restore structures
      structuresCopy.forEach(
        (structure, i) => (this.structures[i] = structure)
      );
    }
...
function typedArrayEncoder(tag, size) {
  ...
  return {
    ...
    encode: function writeExtBuffer(typedArray, encode) {
      ...
      encode(
        hasNodeBuffer
          ? Buffer.from(buffer, offset, length)
...
function writeBundles(start, encode) {
  targetView.setUint32(
    bundledStrings.position + start,
    position - bundledStrings.position - start + 1
  ); // the offset to bundle
